# =============================================================================
# WSC Application: B-Stars Visualization (Exp Transform)
# =============================================================================
#
# Visualizes WSC cross-fit results with exponential transformation.
# Creates: line plot by nc, forest plot at nc=3.
#
...
bstars.df <- full.df %>% 
  ...
  dplyr::ungroup() %>% 
  dplyr::filter(trans == "log", est == "bal.wgt", feat_rep == "raw" | nc == 3) %>% 
  dplyr::mutate(
    feat_group = dplyr::case_when(
      feat_rep %in% c("kbal_only", "rf_only", "bart_only") ~ "Kernel Only",
      feat_rep %in% c("kbal_plus","rf_plus", "bart_plus") ~ "Kernel + Raw",
