...
load("results/lalonde-xfit-kbal.RData")
...
df_plot$trans <- factor(df_plot$trans, levels = c("Untransformed", "Transformed"))
...
df_plot %>% 
  ...
  ggplot2::geom_hline(
    aes(yintercept = bench_est, color = "Exp. Benchmark"),
    inherit.aes = FALSE
  ) +
...
# ggsave(
#   filename = "paper-figs/lalonde_main.pdf",
